            ...
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                ...
                <table className="min-w-full table-auto">
                    ...
                    {[...filteredData].reverse().map((roomPayRec,index)=>(
                        <tr key={roomPayRec.id} className="border-b border-gray-100 hover:bg-gray-50">
                            <td className="px-6 py-3 text-gray-600 text-sm">
                                {index + 1}
                            </td>
                            <td className="px-6 py-3 text-gray-600 text-sm">
                                Room {roomPayRec.roomNo}
                            </td>
                            <td className="px-6 py-3 text-gray-600 text-sm">
                                {roomPayRec.guest}
                            </td>
                            <td className="px-6 py-3 text-gray-600 text-sm">
                                Rs {roomPayRec.finalPayment}
                            </td>
                            <td className="px-6 py-3 text-gray-600 text-sm">
                                {roomPayRec.paymentType}
                            </td>
                            <td className="px-6 py-3 text-gray-600 text-sm">
                                {roomPayRec.date}
                            </td>
